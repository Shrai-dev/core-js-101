...
function encodeToRot13(str) {
  const originAlphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  ...
  for (let i = 0; i < str.length; i++) {
    if (str[i] === ' ') {
      result += ' ';
    } else if (symbols.includes(str[i])) {
      result += str[i];
    } else {
      result += cipheredAlphabet.charAt(originAlphabet.indexOf(str[i]));
    }
  }
  return result;
}
